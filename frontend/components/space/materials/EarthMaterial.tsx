...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // Create continents
    float continents = fbm(sp * 3.0);
    continents = smoothstep(0.4, 0.6, continents);
    
    // Create cloud patterns
    float clouds = fbm(sp * 2.5 + vec3(t * 0.2, t * 0.1, -t * 0.15));
    clouds = smoothstep(0.4, 0.8, clouds);
    
    // Mix ocean and land
    vec3 col = mix(uOcean, uLand, continents);
    
    // Add clouds
    col = mix(col, uCloud, clouds * 0.6);
...
export function EarthMaterial() {
  ...
  const uniforms = useMemo(() => ({
    uTime: { value: 0 },
    uOcean: { value: new THREE.Color('#1a66cc') },
    uLand: { value: new THREE.Color('#3d8e33') },
