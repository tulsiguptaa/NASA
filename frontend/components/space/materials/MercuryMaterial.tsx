...
/*
  MercuryMaterial: Procedural cratered appearance with a gray-silver color palette
  - No external textures required
  - Simulates craters and surface details
*/
...
const fragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // Base noise for surface texture
    float n = fbm(sp * 8.0);
    
    // Create several craters of different sizes
    float craters = 0.0;
    ...
    // Combine noise and craters
    float surface = n * 0.5 + craters;
    
    // Color based on surface features
...
export function MercuryMaterial() {
  ...
  useFrame((_, dt) => { 
    if (ref.current) uniforms.uTime.value += dt; 
  });
